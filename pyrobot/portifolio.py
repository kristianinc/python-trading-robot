...
class Portfolio():
    ...
    def add_position(self, symbol: str, asset_type: str, purchase_date:Optional[str], quantity: int = 0, purchase_price: flaot = 0.0)->dict:
    ...
    def add_positions(self, positons: List[dict]) -> dict:

        if isinstance(positons, List):
    ...
    def remove_position(self, symbol:str)->tuple[bool,str]:
        ...
        if symbol in self.positions:
            ...
            return (False, "{Symbol} did not exist in the portifolio.".format(symbol=symbol))
    ...
    def is_profitable(self, symbol: str, current_price: float) -> bool:
        ...
        #grab the purchse price
        # i think this is only calculating for long positions, not shorts
        # so this definitely needs improvement
